...
/**
 * EMVパーサーユーティリティクラス
 * TLV（Tag-Length-Value）構造のEMVデータをパースする
 */
class EmvParser
{
    /**
     * EMVデータをTLV構造でパースする
     *
     * @param string $emvData 16進数文字列のEMVデータ
     * @return array パースされたTLVデータの配列
     */
    public static function parseEmvData(string $emvData): array
    {
        $result = [];
        $offset = 0;
        $dataLength = strlen($emvData);

        while ($offset < $dataLength) {
            $parsed = self::parseTlv($emvData, $offset);
            if ($parsed === null) {
                break;
            }
     ...
     * @return array|null パースされたTLVデータまたはnull
     */
    private static function parseTlv(string $emvData, int $offset): ?array
    {
        $dataLength = strlen($emvData);
        ...
        // タグを取得
        $tagResult = self::parseTag($emvData, $offset);
        if ($tagResult === null) {
            return null;
        }

        $tag = $tagResult['tag'];
        $tagOffset = $tagResult['next_offset'];

        // 長さを取得
        $lengthResult = self::parseLength($emvData, $tagOffset);
        if ($lengthResult === null) {
            return null;
        }

        $length = $lengthResult['length'];
        $lengthOffset = $lengthResult['next_offset'];

        // 値が足りない場合はnullを返す
        if ($lengthOffset + $length * 2 > $dataLength) {
            return null;
        }

        // 値を取得
        $value = substr($emvData, $lengthOffset, $length * 2);

        return [
            'tag' => $tag,
            'length' => $length,
            'value' => $value,
            'next_offset' => $lengthOffset + $length * 2
        ];
    }
     ...
     * @param string $emvData 16進数文字列のEMVデータ
     * @param int $offset 開始オフセット
     * @return array|null タグ情報またはnull
     */
    private static function parseTag(string $emvData, int $offset): ?array
    {
        $dataLength = strlen($emvData);
        ...
        // 最初のバイトを取得
        $firstByte = hexdec(substr($emvData, $offset, 2));
        $lower5Bits = $firstByte & 0x1F; // 下位5ビット

        if ($lower5Bits !== 0x1F) {
            // 1バイトタグ
            return [
                'tag' => substr($emvData, $offset, 2),
            ...
            while ($currentOffset < $dataLength) {
                $nextByte = hexdec(substr($emvData, $currentOffset, 2));
                $tag .= substr($emvData, $currentOffset, 2);
                $currentOffset += 2;
     ...
     * @param string $emvData 16進数文字列のEMVデータ
     * @param int $offset 開始オフセット
     * @return array|null 長さ情報またはnull
     */
    private static function parseLength(string $emvData, int $offset): ?array
    {
        $dataLength = strlen($emvData);
        ...
        if (($firstByte & 0x80) === 0) {
            ...
            // 長い形式（複数バイト）
            $lengthBytes = $firstByte & 0x7F; // 下位7ビットが長さのバイト数
            $currentOffset = $offset + 2;

            if ($currentOffset + $lengthBytes * 2 > $dataLength) {
                return null;
            }

            $length = 0;
            for ($i = 0; $i < $lengthBytes; $i++) {
                $length = ($length << 8) + hexdec(substr($emvData, $currentOffset, 2));
                $currentOffset += 2;
            }
    ...
    /**
     * パースされたEMVデータを見やすく出力する
     *
     * @param array $parsedData パースされたEMVデータ
     * @return string フォーマットされた文字列
     */
    public static function formatEmvData(array $parsedData): string
    {
        $output = '';

        foreach ($parsedData as $tlv) {
            $tag = $tlv['tag'];
            $length = $tlv['length'];
            $value = $tlv['value'];

            // タグの説明を取得
            $tagDescription = self::getTagDescription($tag);

            $output .= sprintf(
                "Tag: %s (%s) - Length: %d - Value: %s\n",
                $tag,
                $tagDescription,
                $length,
                $value
     ...
     * タグの説明を取得する
     *
     * @param string $tag タグ（16進数文字列）
     * @return string タグの説明
     */
    private static function getTagDescription(string $tag): string
    {
        $tagDescriptions = [
        ...
        return $tagDescriptions[$upperTag] ?? 'Unknown Tag';
    }
}
